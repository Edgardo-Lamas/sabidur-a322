...
- **⚠ El campo `image` de un artículo NO se ve dentro de la página.** El hero que encabeza `/articulo/:slug` es fijo para todos: `img/portada-articulo.jpg`, hardcodeado en `src/components/ReadingSection.jsx`. `image` sólo alimenta la vista previa al compartir (OG y JSON-LD), así que una imagen propia por artículo se ve en WhatsApp y no en el sitio.
